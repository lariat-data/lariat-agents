...
ATHENA_QUERY_BUCKET_NAME = os.getenv(
    ...
    "lariat-athena-default-query-results20230628125818698400000006",
)
...
class AthenaAgent(BatchBaseAgent):
    ...
    def tmp_setup_and_execute_write(self, query_execution_id: str):
        """
        ...
        result_output_path = response["QueryExecution"]["ResultConfiguration"][
            "OutputLocation"
        ].removeprefix("s3:/")
        ...
        indicator_statuses = self.query_builder.construct_indicator_statuses_from_meta(
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("~/Desktop/list_of_ids.csv", header=None)
    print(len(df))
    for query_execution_id in df[1]:
        print(query_execution_id)
        agent.tmp_setup_and_execute_write(query_execution_id=query_execution_id)
